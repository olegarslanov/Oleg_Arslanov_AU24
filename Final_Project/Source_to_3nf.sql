...
ALTER SEQUENCE bl_3nf.seq_ce_promotions RESTART WITH 1;
...
select count(*), promo_id from bl_3nf.ce_promotions group by promo_id having count(*) >1;
...
ALTER SEQUENCE bl_3nf.seq_ce_customers_scd RESTART WITH 1;
...
--delete duplicates row (because it is join many times with cleaned_taxi_data, where is very many rows 'n.a.')
SELECT customer_src_id, customer_telephone, COUNT(*)
FROM bl_3nf.ce_customers_scd
GROUP BY customer_src_id, customer_telephone
HAVING COUNT(*) > 1;


DELETE FROM bl_3nf.ce_customers_scd
WHERE customer_src_id = 'n.a.' and customer_telephone = 'n.a.';
...
ALTER SEQUENCE bl_3nf.seq_ce_bookings RESTART WITH 1;
...
ALTER SEQUENCE bl_3nf.seq_ce_payments RESTART WITH 1;
...
ALTER SEQUENCE bl_3nf.seq_ce_rates RESTART WITH 1;
...
ALTER SEQUENCE bl_3nf.seq_ce_locations RESTART WITH 1;
...
ALTER SEQUENCE bl_3nf.seq_ce_vendor_addresses RESTART WITH 1;
...
ALTER SEQUENCE bl_3nf.seq_ce_vendors RESTART WITH 1;
...
SELECT customer_src_id, customer_id, customer_telephone, start_dt
FROM bl_3nf.ce_customers_scd
WHERE is_active = true;

--drop materialized view bl_3nf.mv_customer_ids;

CREATE MATERIALIZED VIEW bl_3nf.mv_promo_ids AS
SELECT promo_src_id, promo_id
FROM bl_3nf.ce_promotions;
...
--for refresh materilized views
REFRESH MATERIALIZED VIEW bl_3nf.mv_vendor_ids;
REFRESH MATERIALIZED VIEW bl_3nf.mv_booking_ids;
REFRESH MATERIALIZED VIEW bl_3nf.mv_customer_ids;
REFRESH MATERIALIZED VIEW bl_3nf.mv_promo_ids;
REFRESH MATERIALIZED VIEW bl_3nf.mv_payment_ids;
REFRESH MATERIALIZED VIEW bl_3nf.mv_rate_ids;
REFRESH MATERIALIZED VIEW bl_3nf.mv_location_ids;


-- Create sequence
CREATE SEQUENCE IF NOT EXISTS bl_3nf.seq_ce_taxi_trip;
ALTER SEQUENCE bl_3nf.seq_ce_taxi_trip RESTART WITH 1;


--here I create all index for index scan, not sequental .. queries works faster
...
BEGIN
	...
	-- B) insert values in table
		...
		LEFT JOIN customer_ids c ON ctd.customer_src_id = c.customer_src_id and ctd.customer_telephone = c.customer_telephone    -- here important that unique = customer_src_id + customer_telephone
		...
		--LIMIT 100000
		on conflict (trip_src_id) do nothing;
	END IF;
...
----------------------------------------------------------------------------
	...
	WITH 
    vendor_ids AS (
        SELECT DISTINCT ON (vendor_src_id) vendor_src_id, vendor_id
        FROM bl_3nf.mv_vendor_ids
    ),
    booking_ids AS (
        SELECT DISTINCT ON (booking_src_id) booking_src_id, booking_id
        FROM bl_3nf.mv_booking_ids
    ),
    customer_ids AS (
        SELECT DISTINCT ON (customer_src_id) customer_src_id, customer_id, start_dt
        FROM bl_3nf.mv_customer_ids
    ),
    promo_ids AS (
        SELECT DISTINCT ON (promo_src_id) promo_src_id, promo_id
        FROM bl_3nf.mv_promo_ids
    ),
    payment_ids AS (
        SELECT DISTINCT ON (payment_src_id) payment_src_id, payment_id
        FROM bl_3nf.mv_payment_ids
    ),
    rate_ids AS (
        SELECT DISTINCT ON (rate_src_id) rate_src_id, rate_id
        FROM bl_3nf.mv_rate_ids
    ),
    pickup_locations AS (
        SELECT DISTINCT ON (longitude, latitude) longitude, latitude, location_id
        FROM bl_3nf.mv_location_ids
    ),
	...
	SET 
	    ...
	    customer_id = nd.customer_id,
	    promo_id = nd.promo_id,
	    payment_id = nd.payment_id,
	    rate_id = nd.rate_id,
	    pickup_location_id = nd.pickup_location_id,
	    dropoff_location_id = nd.dropoff_location_id,
	    pickup_datetime = nd.pickup_datetime,
	    dropoff_datetime = nd.dropoff_datetime,
	    distance_miles = nd.distance_miles,
	    trip_duration = nd.trip_duration,
	    passenger_count = nd.passenger_count,
	    source_system = nd.source_system,
	    source_entity = nd.source_entity,
	    customer_start_dt = nd.customer_start_dt,
	    update_dt = nd.update_dt
	FROM new_data nd
	WHERE ctt.trip_src_id = nd.trip_src_id
	AND (
	    ctt.distance_miles    IS DISTINCT FROM nd.distance_miles
	    OR ctt.trip_duration  IS DISTINCT FROM nd.trip_duration
	    OR ctt.passenger_count IS DISTINCT FROM nd.passenger_count
	    OR ctt.source_system  IS DISTINCT FROM nd.source_system
	    OR ctt.source_entity  IS DISTINCT FROM nd.source_entity
	);
...
--func
CREATE OR REPLACE procedure bl_3nf.load_ce_taxi_trips2(load_type TEXT)
AS $$
DECLARE
    v_start_time TIMESTAMP := clock_timestamp();
	v_rows_affected int;
BEGIN
	...
	-- B) insert values in batches
	for i in 1..1000 loop
		raise notice 'Processing batch %', i;
		...
		commit;
			...
			raise notice 'Batch % completed. Rows affected: %', i, v_rows_affected;
		End loop;
	END IF;
...
----------------------------------------------------------------------------
	...
	LEFT JOIN vendor_ids v ON ctd.vendor_src_id = v.vendor_src_id
	LEFT JOIN booking_ids b ON ctd.booking_src_id = b.booking_src_id
	LEFT JOIN customer_ids c ON ctd.customer_src_id = c.customer_src_id
	LEFT JOIN promo_ids p ON ctd.promo_src_id = p.promo_src_id
	LEFT JOIN payment_ids py ON ctd.payment_src_id = py.payment_src_id
	LEFT JOIN rate_ids r ON ctd.rate_src_id = r.rate_src_id
	LEFT JOIN pickup_locations pl ON ctd.pickup_longitude = pl.longitude AND ctd.pickup_latitude = pl.latitude
	LEFT JOIN dropoff_locations dl ON ctd.dropoff_longitude = dl.longitude AND ctd.dropoff_latitude = dl.latitude
	WHERE ctt.trip_src_id = ctd.trip_src_id
	  AND (
	...
	LEFT JOIN pickup_locations pl ON ctd.pickup_longitude = pl.longitude AND ctd.pickup_latitude = pl.latitude
	LEFT JOIN dropoff_locations dl ON ctd.dropoff_longitude = dl.longitude AND ctd.dropoff_latitude = dl.latitude
	LEFT JOIN bl_3nf.ce_taxi_trips t ON ctd.trip_src_id = t.trip_src_id
	WHERE t.trip_src_id IS NULL 
	  AND (ctd.pickup_datetime < ctd.dropoff_datetime 
	     OR ctd.pickup_datetime IS NULL 
	     OR ctd.dropoff_datetime IS NULL)
