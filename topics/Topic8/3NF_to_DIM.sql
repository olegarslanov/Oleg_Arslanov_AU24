...
--unique index
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_vendor_src_id
ON bl_dm.dim_vendors (vendor_src_id);
...
BEGIN
    ...
    VALUES ('load_dim_vendors', v_start_time, 'STARTED');
    ...
    VALUES ('load_dim_vendors', v_start_time, now(), v_rows_affected, 'SUCCESS');
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        VALUES ('load_dim_vendors', v_start_time, now(), 0, 'FAILED', SQLERRM);
...
--create unique index;
--CREATE UNIQUE INDEX IF NOT EXISTS idx_dim_customers_scd_unique
--ON bl_dm.dim_customers_scd(customer_src_id, customer_telephone);

--DROP INDEX IF EXISTS idx_dim_customers_scd_unique;
...
BEGIN
    -- Record the start of the procedure in the log
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, status)
    VALUES ('load_dim_customers_scd', v_start_time, 'STARTED');
...
';
    ...
    -- Record the end of the procedure in the log
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status)
    VALUES ('load_dim_customers_scd', v_start_time, now(), v_rows_affected, 'SUCCESS');
...
EXCEPTION
    WHEN OTHERS THEN
        -- Log the error if something goes wrong
        INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status, error_message)
        VALUES ('load_dim_customers_scd', v_start_time, now(), 0, 'FAILED', SQLERRM);
        RAISE NOTICE 'Error occurred: %', SQLERRM;
END;
$$;
...
BEGIN
    ...
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, status)
    VALUES ('load_dim_rates', v_start_time, 'STARTED');
    ...
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status)
    VALUES ('load_dim_rates', v_start_time, now(), v_rows_affected, 'SUCCESS');
...
EXCEPTION
    WHEN OTHERS THEN
        -- Log the error if something goes wrong
        INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status, error_message)
        VALUES ('load_dim_rates', v_start_time, now(), 0, 'FAILED', SQLERRM);
        RAISE NOTICE 'Error occurred: %', SQLERRM;
END;
$$;
...
BEGIN
    -- Log the start of the procedure
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, status)
    VALUES ('load_dim_promotions', v_start_time, 'STARTED');
    ...
    -- Log the successful completion of the procedure
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status)
    VALUES ('load_dim_promotions', v_start_time, now(), v_rows_affected, 'SUCCESS');
...
EXCEPTION
    WHEN OTHERS THEN
        -- Log the error if something goes wrong
        INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status, error_message)
        VALUES ('load_dim_promotions', v_start_time, now(), 0, 'FAILED', SQLERRM);
        RAISE NOTICE 'Error occurred: %', SQLERRM;
END;
$$;
...
BEGIN
    -- Log the start of the procedure
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, status)
    VALUES ('load_dim_locations', v_start_time, 'STARTED');
    ...
    -- Log the successful completion of the procedure
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status)
    VALUES ('load_dim_locations', v_start_time, now(), v_rows_affected, 'SUCCESS');
...
EXCEPTION
    WHEN OTHERS THEN
        -- Log the error if something goes wrong
        INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status, error_message)
        VALUES ('load_dim_locations', v_start_time, now(), 0, 'FAILED', SQLERRM);
        RAISE NOTICE 'Error occurred: %', SQLERRM;
END;
$$;
...
BEGIN
    -- Log procedure start
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, status)
    VALUES ('load_dim_junk_attributes', v_start_time, 'STARTED');
    ...
    INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status)
    VALUES ('load_dim_junk_attributes', v_start_time, now(), v_rows_affected, 'SUCCESS');
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        INSERT INTO bl_dm.load_logs (procedure_name, start_time, end_time, rows_affected, status, error_message)
        VALUES ('load_dim_junk_attributes', v_start_time, now(), 0, 'FAILED', SQLERRM);
        RAISE NOTICE 'Error occurred: %', SQLERRM;
END;
$$;
